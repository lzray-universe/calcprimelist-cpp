...
- Fedora / CentOS / RHEL: `sudo dnf install -y libzstd-devel`
- Arch: `sudo pacman -S zstd`
- macOS (Homebrew): `brew install zstd`
- Windows:
  - vcpkg: `vcpkg install zstd:x64-windows`
...
* Executable: `calcprimelist` (Windows: `calcprimelist.exe`)
* Executable (linked against the static project library): `calcprimelist-static`
* Static library: `calcprime` (and a same-named `calcprime_static`)
* Shared library (exports a C ABI for DLL/FFI): `calcprime-cli`

> Note: `--help` still shows the command prefix as `prime-sieve`; when built from this repo, use `calcprimelist` (or `calcprimelist.exe` on Windows).
...
calcprimelist --from A --to B [options]
  ...
  --wheel-bitmap      Force the wheel-bitmap counting path
...
> `meissel_count` requires a prime table that covers `sqrt(to)`, which can be generated by `simple_sieve`.
...
// Per-segment access and flattened copy
...
// B) Copy all collected primes into a user buffer (set buffer=NULL to query out_written only)
...
> `calcprimelist --to {N} --count --time` 10 times and report **best / median / mean**.
> Windows equivalent: `.\calcprimelist.exe --to {N} --count --time`.
...
> You can also run `calcprimelist --stest` to generate this table automatically.
