...
PrimeWriter::PrimeWriter(bool enabled,
    ...
    : enabled_(enabled),
      ...
      format_(format),
      previous_prime_(0),
      zstd_context_(nullptr),
      zstd_stream_finished_(false),
      io_error_(false) {
    if (!enabled_) {
        return;
    }
    ...
    if (format_ == PrimeOutputFormat::ZstdDelta) {
        ...
        std::size_t out_capacity = ZSTD_CStreamOutSize();
        if (out_capacity == 0) {
            out_capacity = 1;
        }
        zstd_output_buffer_.resize(out_capacity);
    }
...
void PrimeWriter::zstd_stream_compress(std::string_view data) {
    if (data.empty()) {
        return;
    }
    if (!zstd_context_) {
        throw std::runtime_error("Zstd compression context is not initialized");
    }
    if (zstd_stream_finished_) {
        throw std::runtime_error("Cannot compress after zstd stream has finished");
    }

    ZSTD_inBuffer input{data.data(), data.size(), 0};
    while (input.pos < input.size) {
        ZSTD_outBuffer output{zstd_output_buffer_.data(), zstd_output_buffer_.size(), 0};
        size_t rc = ZSTD_compressStream2(zstd_context_, &output, &input, ZSTD_e_continue);
        if (ZSTD_isError(rc)) {
            throw std::runtime_error(std::string("ZSTD_compressStream2 failed: ") +
                                     ZSTD_getErrorName(rc));
        }
        if (output.pos > 0) {
            enqueue_chunk(Chunk{std::string(zstd_output_buffer_.data(), output.pos), false});
        }
    }
}

void PrimeWriter::zstd_stream_finish() {
    if (!zstd_context_ || zstd_stream_finished_) {
        return;
    }
    ...
    for (;;) {
        ZSTD_outBuffer output{zstd_output_buffer_.data(), zstd_output_buffer_.size(), 0};
        size_t rc = ZSTD_compressStream2(zstd_context_, &output, &input, ZSTD_e_end);
        if (ZSTD_isError(rc)) {
            throw std::runtime_error(std::string("ZSTD_compressStream2 failed: ") +
                                     ZSTD_getErrorName(rc));
        }
