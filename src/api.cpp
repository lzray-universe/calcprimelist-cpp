...
to_c_segment_config(const calcprime::SegmentConfig&config){
...
extern "C" int calcprime_simple_sieve(std::uint64_t limit,
...
extern "C" calcprime_segment_config
calcprime_choose_segment_config(const calcprime_cpu_info*info,unsigned threads,
	...
	auto cpp_config=calcprime::choose_segment_config(
		cpp_info,threads,requested_segment_bytes,requested_tile_bytes,
		range_length);
...
enum class FailureKind{ None,Writer,PrimeCallback,Progress };

calcprime_status assign_exception(calcprime_range_run_result&result,
								  FailureKind kind,
...
calcprime_run_range(const calcprime_range_options*options,
	...
	calcprime::SegmentConfig config=calcprime::choose_segment_config(
		cpu_info,threads,opts.segment_bytes,opts.tile_bytes,length);
	...
	std::uint64_t sqrt_limit=static_cast<std::uint64_t>(
	...
	auto deliver_chunk=[&](std::vector<std::uint64_t>&&chunk) -> bool{
		...
		if(opts.prime_callback){
			...
			try{
				callback_result=opts.prime_callback(chunk.data(),chunk.size(),
													opts.prime_user_data);
			}catch(...){
				if(failure_kind==FailureKind::None){
	...
	if(opts.progress_callback&&!progress_cancelled&&
	   failure_kind==FailureKind::None&&!external_cancelled){
		try{
	...
	bool completed=(processed==num_segments)&&!cancelled&&
...
extern "C" int calcprime_range_result_segment(
...
extern "C" std::uint64_t calcprime_count_zero_bits(const std::uint64_t*bits,
												   std::size_t bit_count){
	if(!bits){
		return 0;
	}
