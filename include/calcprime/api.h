...
typedef int (*calcprime_prime_chunk_callback)(const std::uint64_t*primes,
...
/**
 * Runs the command-line interface of calcprimelist using the provided
 * arguments. This exposes the same behaviour as the prime-sieve executable.
 *
...
/**
 * Counts primes in the half-open interval [from, to) using the
 * Meissel–Lehmer algorithm. A thread count of 0 selects the
 * hardware concurrency reported by the system.
 */
CALCPRIME_API std::uint64_t
calcprime_meissel_count(std::uint64_t from,std::uint64_t to,unsigned threads);

/**
 * Performs a deterministic Miller-Rabin primality check for a
 * 64-bit integer. Returns 1 for prime numbers, 0 otherwise.
 */
CALCPRIME_API int calcprime_miller_rabin_is_prime(std::uint64_t n);

/**
 * Computes all prime numbers up to the inclusive limit using the
 * simple sieve. The resulting array is allocated by the library
 * and must be freed with calcprime_release_u32_buffer.
 *
 * @param limit The inclusive upper bound of the sieve.
 * @param out_primes Receives the pointer to the allocated buffer.
 * @param out_count Receives the number of primes written to the buffer.
 * @return 0 on success, non-zero on invalid arguments or allocation failure.
 */
CALCPRIME_API int calcprime_simple_sieve(std::uint64_t limit,
										 std::uint32_t**out_primes,
										 std::size_t*out_count);

/**
 * Releases a buffer that was allocated by the calcprime_simple_sieve
 * function.
 */
CALCPRIME_API void calcprime_release_u32_buffer(std::uint32_t*buffer);

/**
 * Counts primes in [from, to) using a precomputed table of primes.
 * The table must contain all primes up to sqrt(to).
 */
CALCPRIME_API std::uint64_t
...
/**
 * Detects CPU information such as core counts and cache sizes.
 */
CALCPRIME_API calcprime_cpu_info calcprime_detect_cpu_info(void);

/**
 * Computes the effective number of worker threads to use based on the
 * detected CPU information.
 */
CALCPRIME_API unsigned
calcprime_effective_thread_count(const calcprime_cpu_info*info);

/**
 * Chooses an appropriate segment configuration for the provided parameters.
 */
CALCPRIME_API calcprime_segment_config calcprime_choose_segment_config(
	const calcprime_cpu_info*info,unsigned threads,
	std::size_t requested_segment_bytes,std::size_t requested_tile_bytes,
...
CALCPRIME_API calcprime_status
calcprime_run_range(const calcprime_range_options*options,
					calcprime_range_run_result**out_result);

CALCPRIME_API calcprime_status
calcprime_range_result_status(const calcprime_range_run_result*result);

CALCPRIME_API const char*
calcprime_range_result_error_message(const calcprime_range_run_result*result);

CALCPRIME_API std::uint64_t
calcprime_range_result_count(const calcprime_range_run_result*result);

CALCPRIME_API int
calcprime_range_result_nth_prime(const calcprime_range_run_result*result,
								 std::uint64_t*out_value);

CALCPRIME_API int
calcprime_range_result_stats(const calcprime_range_run_result*result,
							 calcprime_range_stats*out_stats);

CALCPRIME_API std::size_t
calcprime_range_result_segment_count(const calcprime_range_run_result*result);

CALCPRIME_API int calcprime_range_result_segment(
	const calcprime_range_run_result*result,std::size_t index,
	const std::uint64_t**out_primes,std::size_t*out_count);

CALCPRIME_API int
calcprime_range_result_copy_primes(const calcprime_range_run_result*result,
								   std::uint64_t*buffer,std::size_t capacity,
								   std::size_t*out_written);

CALCPRIME_API void
calcprime_range_result_release(calcprime_range_run_result*result);

/**
 * Counts the set bits of a 64-bit value.
 */
CALCPRIME_API std::uint64_t calcprime_popcount_u64(std::uint64_t value);

/**
 * Counts the zero bits in a buffer containing packed bit flags.
 */
CALCPRIME_API std::uint64_t calcprime_count_zero_bits(const std::uint64_t*bits,
